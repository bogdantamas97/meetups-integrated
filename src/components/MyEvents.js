...
import FutureEventItem from "./future-events-list/FutureEventItem";
import Cookies from "universal-cookie";
import { EventsMessage, CloseMessageButton } from "./EventsMessage";
import { eventType } from "./Enumerations";
import moment from "moment/moment";
import EventDialog from "./future-events-list/EventDialog";
...
class MyEvents extends React.Component {
  ...
  isUsersEvent = event => {
    let id=parseInt(new Cookies().get("token").substring(6));
    return event.attendanceIds.includes(id)||event.waitingListIds.includes(id);
  };

  componentDidMount() {
    let isMessageVisible = !this.state.cookies.get("myEventsMessageClosed");
